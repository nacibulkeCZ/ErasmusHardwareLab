    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                // Animated effects only active when highlight is significant
                ...
                bodyColor.rgb = lerp(bodyColor.rgb, fillGlow.rgb, fillMask);
                bodyColor.rgb += fillHead * _VignetteColor.rgb * 0.65 * fillMask;
                bodyColor.rgb += frameGlow.rgb * borderMask;
                bodyColor.rgb += _VignetteColor.rgb * _VignetteIntensity * fillMask * 0.10;

                half4 finalColor = texColor;
                finalColor.rgb *= bodyColor.rgb;
                finalColor.rgb += (fillHead * _VignetteColor.rgb + sweep * _ActiveBorderColor.rgb) * fillMask * 0.18;
